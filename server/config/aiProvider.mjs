...
const provider = normalizeProvider(process.env.SYNTHESIS_API_PROVIDER ?? "anthropic");
...
const anthropicFastModel = resolveModel(process.env.ANTHROPIC_FAST_MODEL, "claude-haiku-4-5-20251001");
const openaiDefault = resolveModel(
  process.env.OPENAI_MODEL ??
    (provider === "openai" ? process.env.SYNTHESIS_MODEL : undefined),
  "gpt-4o",
);
const openaiFastModel = resolveModel(process.env.OPENAI_FAST_MODEL, "gpt-4o-mini");
...
const CONFIGS = {
  anthropic: {
    ...
    anthropicVersion: String(process.env.ANTHROPIC_VERSION ?? "2023-06-01").trim() || "2023-06-01",
  },
  openai: {
